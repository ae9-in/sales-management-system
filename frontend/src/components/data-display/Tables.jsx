...
const Table = ({
  ...
  const renderState = (Icon, message) => (
    <div className="px-4 py-12 text-center bg-gradient-to-b from-white to-gray-800">
      <Icon className={`w-8 h-8 mx-auto mb-4 text-gray-500 ${Icon === Loader2 ? "animate-spin" : ""}`} />
  ...
  const renderTableRow = (row, i) => (
    <div key={i} className="grid transition-all hover:bg-white/50" style={{ gridTemplateColumns: getGridTemplate() }}>
      {columns.map(col => (
        <div key={col.key} className="px-4 py-3 text-sm text-gray-600 truncate">
  ...
  const renderMobileRow = (row, i) => (
    <div key={i} className="p-4 transition-all hover:bg-white/50">
      {columns.map(col => (
        <div key={col.key} className="flex justify-between py-1">
          <span className="text-sm font-medium text-gray-500">{col.label}:</span>
          <span className="text-sm text-gray-600">{formatCellValue(row[col.key], col)}</span>
        </div>
      ))}
  ...
  return (
    <div className="overflow-hidden border rounded-lg shadow-xl bg-gradient-to-br from-white via-gray-50 to-white border-gray-200/50">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b-2 border-gray-200 shadow-md">
        <h2 className="text-lg font-medium text-gray-900">Records</h2>
        <div className="px-3 py-1 text-sm font-medium text-gray-600 rounded-md bg-gray-100/50">
