...
const getIcon = (type) => {
  switch (type) {
    ...
    case 'meeting': return { icon: Calendar, color: 'text-blue-400 bg-blue-500/10' };
    case 'payment-received': return { icon: Award, color: 'text-yellow-400 bg-yellow-500/10' };
    case 'stock-alert': return { icon: ShieldAlert, color: 'text-blue-400 bg-blue-500/10' };
    case 'customer-added': return { icon: UserCheck, color: 'text-pink-400 bg-pink-500/10' };
    case 'report-ready': return { icon: FileSpreadsheet, color: 'text-gray-400 bg-gray-500/10' };
    case 'task-completed': return { icon: Check, color: 'text-green-400 bg-green-500/10' };
    default: return { icon: CreditCard, color: 'text-gray-400 bg-gray-500/10' };
  }
};
...
const NotificationList = ({ notifications = [], activeTab, setActiveTab, onToggleRead }) => {
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 flex-1 flex flex-col overflow-hidden">
      {/* Tabs Header */}
      <div className="flex border-b border-gray-700 mb-4 overflow-x-auto no-scrollbar whitespace-nowrap">
        {tabs.map((tab, i) => (
          <button
            key={i}
          ...
          >
            {tab.name}
            <span className={`px-1.5 py-0.5 rounded text-[10px] ${activeTab === tab.name ? 'bg-blue-600/20 text-blue-400' : 'bg-gray-700 text-gray-400'}`}>
              {tab.count}
            </span>
          </button>
        ))}
        ...
        {filteredNotifications.map((notif) => {
          ...
          return (
            <div
              ...
              className={`p-4 rounded-xl border flex items-start gap-4 transition cursor-pointer hover:bg-gray-700/20 ${
                notif.isUnread
                  ? 'bg-gray-700/20 border-gray-700/50'
                  : 'bg-transparent border-gray-700/20'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 leading-relaxed mb-3">{notif.description}</p>
                {notif.action && (
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="border border-gray-600 bg-transparent hover:bg-gray-700 text-gray-200 text-xs px-3 py-1.5 rounded-lg transition font-medium"
                  >
                    {notif.action}
                  </button>
                )}
              ...
              <div className="flex flex-col items-end justify-between shrink-0 gap-2">
                ...
                {notif.isUnread && <div className="w-2 h-2 rounded-full bg-blue-500"></div>}
