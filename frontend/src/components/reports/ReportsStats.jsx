...
const StatCard = ({ title, amount, subtext, change, isPositive, icon: Icon, colorClass }) => (
  <div className="flex flex-col justify-between p-4 bg-white border border-gray-200 rounded-xl">
    ...
    <div>
      ...
      <h3 className="text-xl font-bold text-gray-900 mb-1">{amount}</h3>
...
const ReportsStats = ({ sales = [] }) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 lg:grid-cols-5 mb-6">
      <StatCard title="Total Revenue" amount={`₹${totalRevenue.toLocaleString()}`} subtext={totalRevenue > 0 ? "vs previous period" : "No sales recorded"} change={totalRevenue > 0 ? "12.4%" : ""} isPositive={true} icon={IndianRupee} colorClass="bg-emerald-500/20 text-emerald-600" />
